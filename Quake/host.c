...
void Host_InitLocal (void)
{
	...
	Max_Fps_f(&host_maxfps); // Cvar_SetCallback doesn't invoke; init host_netinterval so renderer/network isolation engages before any config touches host_maxfps
...
// config.cfg cvars
	if (host_initialized && !isDedicated && !host_parms->errstate)
	{
		...
		f = fopen (va("%s/%s", config_dir, name), "w");
		if (!f)
		{
...
// process console commands
	...
	//if (pipe_handle != INVALID_HANDLE_VALUE && Pipe_AvailableBytes() > 0) {
	//	if (Pipe_Read() && Q_strcmp(pipe_buffer, "connect") == 0) {
	//		Cbuf_AddText(pipe_buffer);
	//	}
	//}
...
void Host_AddUserConfigPath(void)
{
	char config_dir[MAX_PATH];
	GetUserConfigDir(config_dir);
	searchpath_t * search = (searchpath_t*)Z_Malloc(sizeof(searchpath_t));
	q_strlcpy(search->filename, config_dir, sizeof(search->filename));
	search->path_id =  0;
	search->pack = NULL;
