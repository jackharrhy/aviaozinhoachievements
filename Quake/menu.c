...
// ===== AutoAim =====
void M_Menu_AutoAim_f(void);
void M_AutoAim_Draw(void);
void M_AutoAim_Key(int key);
void M_AutoAim_Mousemove(int cx, int cy);
...
// ===== Workshop_Mods =====
void M_Menu_Workshop_Mods_f(void);
void M_Workshop_Mods_Draw(void);
void M_Workshop_Mods_Mousemove(int cx, int cy);
...
void M_Help_Load(void);
void M_Help_Key(int key);
void M_Help_Mousemove(int cx, int cy);
void M_Help_GoBack(void);
...
// ===== Lore =====
void M_Menu_Lore_f(void);
void M_Lore_Draw(void);
void M_Lore_Load(void);
void M_Lore_Key(int key);
void M_Lore_Mousemove(int x, int y);
void M_Lore_GoBack(void);
...
// ===== GameOver =====
void M_Menu_GameOver_f(void);
void M_GameOver_Draw(void);
void M_GameOver_Load(void);
void M_GameOver_Key(int key);
...
void M_AllMods_Draw(void);
void M_AllMods_Key(int key);
void M_AllMods_Mousemove(int cx, int cy);

// ==== Language ====
void     M_Menu_Language_f(void);
void     M_Language_Draw(void);
qboolean M_Language_Match(int index, char initial);
void     M_Language_Key(int key);
void     M_Language_Mousemove(int cx, int cy);
...
================
*/
void M_DrawCharacter(int cx, int line, Uint32 num)
{
...
void M_PrintColor(int cx, int cy, const char* str)
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		color.rgb[1] = 255;
		color.rgb[2] = 0;
		M_DrawCharacterRGBA(cx, cy, codepoint, color, 1.0f);
		cx += 8;
	}
}
...
void M_PrintWithBreak(int cx, int cy, const char* str, int max_len, int line_len)
{
	int start_cx = cx;
	int line_chars = 0;
	size_t bytes = strlen(str);
...
void M_DrawCharacterRGBA(int cx, int line, Uint32 num, plcolour_t c, float alpha) // woods
{
...
void M_Main_Draw(void) // woods #modsmenu #demosmenu (iw)
{
...

	{
		...
		glScalef(bannerScale, bannerScale, 1.0f);
		glTranslatef(-halfW, -halfH, 0.0f);
		M_DrawTransPic(0, 0, bannerPic);
		if (m_main_banner_hover) {
			glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	...
	}
	{
		...
		glScalef(bannerScale, bannerScale, 1.0f);
		glRotatef(bannerRotation, 0.0f, 0.0f, 1.0f);
		glTranslatef(-halfW, -halfH, 0.0f);
		M_DrawTransPic(0, 0, bannerPic);
		if (m_main_banner2_hover) {
			glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	...
	{
		M_Print(x * invScale, y * invScale, LOC_GetString("$menu_allmods")); y += 20;
...
static int save_compare(const void* a, const void* b)
{
...
void M_ScanSaves(void)
{
	...
	for (i = 0; i < MAX_SAVEGAMES; i++)
	{
		...
		if (!f)
		{
			strncpy(save_entries[i].name, LOC_GetString("$menu_unused_slot"), sizeof(save_entries[i].name));
			continue;
		}
...
static void M_DrawSaveSlots(const char* title_pic)
{
	...
	{
		char info[128];
		M_Print(16, 32 + 8 * (MAX_SAVEGAMES - 1) + 4, LOC_GetString("$menu_last_save"));
...
void M_Maps_Draw(void)
{
	...
	Draw_String(x, y - 28, LOC_GetString("$menu_maps"));
...
int				m_autoaim_cursor;
qboolean		m_autoaim_usegfx;
qboolean		m_autoaim_usecustomtitle;
int				m_autoaim_numoptions;
char			m_autoaim_mapname[MAX_QPATH];
char			m_autoaim_maptitle[1024];
menuticker_t	m_autoaim_ticker;

enum m_state_e m_autoaim_prevmenu;

void M_Menu_AutoAim_f(void)
{
	key_dest = key_menu;
	m_autoaim_prevmenu = m_state;
	m_state = m_autoaim;
	m_entersound = true;
...
void M_AutoAim_Draw(void)
{
	...
	const char* title = LOC_GetString("$menu_autoaim");
	M_PrintWhite((320 - 8 * strlen(title)) / 2, 4, title);
	...
	for (f = 0; f < 2; f++)
	{
		M_Print(x, y + f * 16 + 2, autoaims[f]);
	}
	if (m_autoaim_cursor < 2)
	{
		M_DrawArrowCursor(x - 16, y + m_autoaim_cursor * 16 + 2);
	}
}

//static double autoaim_last_key_time = 0.0; // Tracks last key time for 'ni' combo
//static qboolean autoaim_was_n = false;    // Tracks if the last key was 'n'

void M_AutoAim_Key(int key)
{
	if (M_Ticker_Key(&m_autoaim_ticker, key))
		return;
	...
	case K_MOUSE4:
	case K_MOUSE2:
		m_state = m_autoaim_prevmenu;
		m_entersound = true;
		break;

	case K_DOWNARROW:
		S_LocalSound("misc/menu1.wav");
		if (++m_autoaim_cursor > m_autoaim_numoptions - 1)
			m_autoaim_cursor = 0;
		break;

	case K_UPARROW:
		S_LocalSound("misc/menu1.wav");
		if (--m_autoaim_cursor < 0)
			m_autoaim_cursor = m_autoaim_numoptions - 1;
		break;
...
void M_AutoAim_Mousemove(int cx, int cy)
{
	...
	M_UpdateCursor(cy, ybase, itemheight, 2, &m_autoaim_cursor);
...
void M_MultiPlayer_Draw(void)
{
	...
	if (ipxAvailable || ipv4Available || ipv6Available)
		return;
	M_PrintWhite((320 / 2) - ((27 * 8) / 2), 148, LOC_GetString("$menu_no_comm_avail"));
}
...
void M_AllMods_Draw(void)
{
	...
	const char* title = LOC_GetString("$menu_allmods");
	M_PrintWhite((320 - 8 * strlen(title)) / 2, 4, title);
...
void M_AllMods_Key(int key)
{
	int allItems = pipe_available ? ALLMODS_ITEMS : 1;
	switch (key)
	{
	case K_ESCAPE:
	case K_BBUTTON:
...
void M_AllMods_Mousemove(int cx, int cy) // woods #mousemenu
{
	int allItems = pipe_available ? ALLMODS_ITEMS : 1;
	M_UpdateCursor(cy, 32, 20, allItems, &m_allmods_cursor);
}
...
static void M_Language_Add(const char* name, const char* filename)
{
	...
	VEC_PUSH(languagesmenu.items, language);

	languagesmenu.items[languagesmenu.list.numitems] = language;
	languagesmenu.list.numitems++;
}

static void M_Language_Init(void)
{
	filelist_item_t* item;

	languagesmenu.list.viewsize = MAX_VIS_LANGUAGES;
	languagesmenu.list.cursor = -1;
	languagesmenu.list.scroll = 0;
	languagesmenu.list.numitems = 0;
	languagesmenu.modcount = 0;
	languagesmenu.scrollbar_grab = false;
	VEC_CLEAR(languagesmenu.items);

	M_Ticker_Init(&languagesmenu.ticker);
	...
	if (languagesmenu.list.cursor == -1)
		languagesmenu.list.cursor = 0;

	M_List_CenterCursor(&languagesmenu.list);
}

void M_Menu_Language_f(void)
{
	key_dest = key_menu;
	languagesmenu.prev = m_state;
	m_state = m_language;
	m_entersound = true;
	M_Language_Init();
}
...
void M_Language_Draw(void)
{
	...
	languagesmenu.x = x;
	languagesmenu.y = y;
	languagesmenu.cols = cols;

	if (!keydown[K_MOUSE1])
		languagesmenu.scrollbar_grab = false;

	if (languagesmenu.prev_cursor != languagesmenu.list.cursor)
	{
		languagesmenu.prev_cursor = languagesmenu.list.cursor;
		M_Ticker_Init(&languagesmenu.ticker);
	}
	else
		M_Ticker_Update(&languagesmenu.ticker);

	Draw_String(x, y - 28, LOC_GetString("$menu_languages"));
	M_DrawQuakeBar(x - 8, y - 16, cols + 2);

	M_List_GetVisibleRange(&languagesmenu.list, &firstvis, &numvis);
	for (i = 0; i < numvis; i++)
	{
		int idx = i + firstvis;
		qboolean selected = (idx == languagesmenu.list.cursor);

		M_PrintScroll(x, y + i * 8, (cols - 2) * 8, languagesmenu.items[idx].name, selected ? languagesmenu.ticker.scroll_time : 0.0, 1);

		if (selected)
			M_DrawCharacter(x - 8, y + i * 8, 12 + ((int)(realtime * 4) & 1));
	}

	if (M_List_GetOverflow(&languagesmenu.list) > 0)
	{
		M_List_DrawScrollbar(&languagesmenu.list, x + cols * 8 - 8, y);

		if (languagesmenu.list.scroll > 0)
			M_DrawEllipsisBar(x, y - 8, cols);
		if (languagesmenu.list.scroll + languagesmenu.list.viewsize < languagesmenu.list.numitems)
			M_DrawEllipsisBar(x, y + languagesmenu.list.viewsize * 8, cols);
	}
}

qboolean M_Language_Match(int index, char initial)
{
	return q_tolower(languagesmenu.items[index].name[0]) == initial;
}
...
void M_Language_Key(int key)
{
	...
	if (languagesmenu.scrollbar_grab)
	{
		switch (key)
		{
		case K_ESCAPE:
	...
	if (M_List_Key(&languagesmenu.list, key))
		return;

	if (M_List_CycleMatch(&languagesmenu.list, key, M_Language_Match))
		return;

	if (M_Ticker_Key(&languagesmenu.ticker, key))
		return;
	...
	case K_MOUSE1: // woods #mousemenu
		x = m_mousex - languagesmenu.x - (languagesmenu.cols - 1) * 8;
		y = m_mousey - languagesmenu.y;
		if (x < -8 || !M_List_UseScrollbar(&languagesmenu.list, y))
			goto enter;
		languagesmenu.scrollbar_grab = true;
		M_Language_Mousemove(m_mousex, m_mousey);
...
void M_Language_Mousemove(int cx, int cy) // woods #mousemenu
{
	cy -= languagesmenu.y;
	...
	if (languagesmenu.scrollbar_grab)
	{
		...
		M_List_UseScrollbar(&languagesmenu.list, cy);
	}

	M_List_Mousemove(&languagesmenu.list, cy);
}
...
void M_Setup_Draw(void)
{
	...
	M_Print(64, 40, LOC_GetString("$menu_hostname"));
	M_DrawTextBox(160, 32, 16, 1);
	M_Print(168, 40, setup_hostname);

	M_Print(64, 56, LOC_GetString("$menu_your_name"));
	M_DrawTextBox(160, 48, 16, 1);
	M_PrintWhite(168, 56, setup_myname); // woods change to white #namemaker


	//M_Print(64, 72, LOC_GetString("$menu_name_maker")); // woods #namemaker

	M_Print(64, 88, LOC_GetString("$menu_shirt_dash")); // woods 80 to 104 #namemaker #showcolornum
	...
	M_Print(64, 112, LOC_GetString("$menu_pants_dash")); // woods 104 to 128 #namemaker #showcolornum
	...
	if (!rgbactive && (setup_cursor == 3 || setup_cursor == 4)) // woods
		M_PrintRGBA(64, 170, LOC_GetString("$menu_shift_for_rgb_colors"), CL_PLColours_Parse("0xffffff"), 0.6f, false); // woods

	M_DrawTextBox(64, 142, 14, 1);  // woods 140 to 152 #namemaker
	M_Print(72, 150, LOC_GetString("$menu_accept_changes")); // woods #colorbar
...
#define NAMEMAKER_TOTAL_ROWS (NAMEMAKER_TABLE_SIZE + 1)
...
void M_NameMaker_Draw(void)
{
	...
	M_Print(48, 16, LOC_GetString("$menu_your_name"));
	...
	M_PrintWhite(32, 48 + 8 * NAMEMAKER_TABLE_SIZE, LOC_GetString("$menu_web_name_maker"));
...
void M_NameMaker_Key(int k)
{
	...
	case K_MOUSE1: // woods #mousemenu
		...
		else if (namemaker_cursor_y == NAMEMAKER_TABLE_SIZE)
		{
			...
			SCR_ModalMessage(LOC_GetString("$msg_web_name_maker_opened_in_browser"), 3.5f); // woods
...
void M_NameMaker_Mousemove(int cx, int cy) // woods #mousemenu
{
	...
	temp_cursor_x = (cx - 8 - x_origin + x_spacing / 2) / x_spacing;
	...
	if (temp_cursor_y < NAMEMAKER_TABLE_SIZE)
	...
	namemaker_cursor_x = temp_cursor_x;
...
#endif
	if (on)
		M_Print(x, y, LOC_GetString("$menu_on"));
	else
		M_Print(x, y, LOC_GetString("$menu_off"));
}
...
void M_Options_Draw(void)
{
	...
	for (int i = 0; i < OPTIONS_ITEMS; i++)
	{
		...
		switch (i) {
		case OPT_CUSTOMIZE:
			text = LOC_GetString("$menu_keysetup");
			break;
		case OPT_MOUSE:
			text = LOC_GetString("$menu_mouse");
			break;
		case OPT_CONTROLLER:
			text = LOC_GetString("$menu_controller");
			break;
		case OPT_VIDEO:
			if (vid_menudrawfn)
				text = LOC_GetString("$menu_display");
			break;
		case OPT_GRAPHICS:
			if (vid_menudrawfn)
				text = LOC_GetString("$menu_graphics");
			break;
		case OPT_SOUND:
			text = LOC_GetString("$menu_sound");
			break;
		case OPT_GAME:
			text = LOC_GetString("$menu_game");
			break;
		case OPT_HUD:
			text = LOC_GetString("$menu_hud");
			break;
		case OPT_CONSOLEM:
			text = LOC_GetString("$menu_consolem");
			break;
		case OPT_MENUSCALE:
			text = LOC_GetString("$menu_menuscale");
	...
	if (optionsmenu.list.search.len > 0)
	{
		...
		M_PrintHighlight(24, 178, optionsmenu.list.search.text,
			optionsmenu.list.search.text,
			optionsmenu.list.search.len);
		int cursor_x = 24 + 8 * optionsmenu.list.search.len;
...
static const char* M_Options_GetItemText(int index)
{
	...
	{
	case OPT_CUSTOMIZE:
		return LOC_GetString("$menu_keysetup");
	case OPT_MOUSE:
		return LOC_GetString("$menu_mouse");
	case OPT_CONTROLLER:
		return LOC_GetString("$menu_controller");
	case OPT_VIDEO:
		return LOC_GetString("$menu_display");
	case OPT_GRAPHICS:
		return LOC_GetString("$menu_graphics");
	case OPT_SOUND:
		return LOC_GetString("$menu_sound");
	case OPT_GAME:
		return LOC_GetString("$menu_game");
	case OPT_HUD:
		return LOC_GetString("$menu_hud");
	case OPT_CONSOLEM:
		return LOC_GetString("$menu_consolem");
	case OPT_MENUSCALE:
	...
	if (COM_FOpenFile("bindlist.lst", &file, NULL) >= 0 && file)
	{
		while (fgets(line, sizeof(line), file))
		{
			...
			if (*cmd)
			{
				...
				strcpy(bindnames[numbindnames].cmd, cmd); //todo: unsafe
				bindnames[numbindnames].desc = (char*)Z_Malloc(strlen(desc) + 1);
				strcpy(bindnames[numbindnames].desc, desc); //todo: unsafe
				numbindnames++;
			}
		}
		fclose(file);
...
void M_Keys_Draw(void)
{
	...
	if (bind_grab)
		M_Print(12, 32, LOC_GetString("$menu_presskey"));
	else
		M_Print(18, 32, LOC_GetString("$menu_editbind"));
...
extern cvar_t	joy_outer_threshold_look;
extern cvar_t	joy_outer_threshold_move;
...
static enum controller_e
{
	...
	CONTROLLER_OUTER_THRESHOLD_LOOK,
	CONTROLLER_OUTER_THRESHOLD_MOVE,
	CONTROLLER_DEADZONE_TRIGGER,
	CONTROLLER_SENSITIVITY_YAW,
	CONTROLLER_SENSITIVITY_PITCH,
	CONTROLLER_INVERT,
	CONTROLLER_EXPONENT,
	CONTROLLER_EXPONENT_MOVE,
	CONTROLLER_SWAPMOVELOOK,
	CONTROLLER_ENABLE,
	CONTROLLER_COUNT
} controller_cursor;

#define CONTROLLER_ITEMS (CONTROLLER_COUNT)
int numberOfControllerItems = CONTROLLER_ITEMS;
...
float M_ControllerToSliderFraction(int cx) // woods #mousemenu
{
	float f;
	f = (cx - 4) / (float)((SLIDER_RANGE - 1) * 8);
	return CLAMP(0.f, f, 1.f);
...
static const char* M_Controller_GetItemText(int index)
{
	...
	case CONTROLLER_DEADZONE_LOOK:
		return LOC_GetString("$menu_controller_deadzone_look");

	case CONTROLLER_DEADZONE_MOVE:
		return LOC_GetString("$menu_controller_deadzone_move");

	case CONTROLLER_OUTER_THRESHOLD_LOOK:
		return LOC_GetString("$menu_controller_outer_threshold_look");

	case CONTROLLER_OUTER_THRESHOLD_MOVE:
		return LOC_GetString("$menu_controller_outer_threshold_move");

	case CONTROLLER_DEADZONE_TRIGGER:
		return LOC_GetString("$menu_controller_deadzone_trigger");

	case CONTROLLER_SENSITIVITY_YAW:
		return LOC_GetString("$menu_controller_sensitivity_yaw");

	case CONTROLLER_SENSITIVITY_PITCH:
		return LOC_GetString("$menu_controller_sensitivity_pitch");

	case CONTROLLER_EXPONENT:
		return LOC_GetString("$menu_controller_exponent");

	case CONTROLLER_EXPONENT_MOVE:
		return LOC_GetString("$menu_controller_exponent_move");

	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
	}
}
...
void M_Menu_Controller_f(void)
{
	...
	m_entersound = true;
	controller_cursor = 0;
	controllermenu.cursor = 0;
	controllermenu.search.len = 0;
	controllermenu.search.text[0] = 0;
	numberOfControllerItems = CONTROLLER_ITEMS;
...
static void M_Controller_AdjustSliders(int dir)
{
	...
	switch (controller_cursor)
	{
	case CONTROLLER_DEADZONE_LOOK:
		f = joy_deadzone_look.value + dir * 0.01f;
		if (f > 1.0f) f = 1.0f;
	...
	case CONTROLLER_DEADZONE_MOVE:
		f = joy_deadzone_move.value + dir * 0.01f;
		if (f > 1.0f) f = 1.0f;
		else if (f < 0.001f) f = 0.001f;
	...
	case CONTROLLER_OUTER_THRESHOLD_LOOK:
		f = joy_outer_threshold_look.value + dir * 0.01f;
		if (f > 1.0f) f = 1.0f;
		else if (f < 0.001f) f = 0.001f;
		Cvar_SetValue("joy_outer_threshold_look", f);
		break;

	case CONTROLLER_OUTER_THRESHOLD_MOVE:
		f = joy_outer_threshold_move.value + dir * 0.01f;
		if (f > 1.0f) f = 1.0f;
		else if (f < 0.001f) f = 0.001f;
		Cvar_SetValue("joy_outer_threshold_move", f);
		break;

	case CONTROLLER_DEADZONE_TRIGGER:
		f = joy_deadzone_trigger.value + dir * 0.01f;
		if (f > 1.0f) f = 1.0f;
		else if (f < 0.001f) f = 0.001f;
	...
	case CONTROLLER_SENSITIVITY_YAW:
		f = joy_sensitivity_yaw.value + dir * 1.0f;
	...
	case CONTROLLER_SENSITIVITY_PITCH:
		f = joy_sensitivity_pitch.value + dir * 1.0f;
		if (f > 360.0f) f = 360.0f;
		else if (f < 0.001f) f = 0.001f;
	...
	case CONTROLLER_EXPONENT_MOVE:
		f = joy_exponent_move.value + dir * 0.1f;
		if (f > 10.0f) f = 10.0f;
		else if (f < 0.001f) f = 0.001f;
		Cvar_SetValue("joy_exponent_move", f);
		break;
	case CONTROLLER_INVERT:
		Cvar_SetValue("joy_invert", -joy_invert.value);
		break;
	case CONTROLLER_SWAPMOVELOOK:
		Cvar_SetValue("joy_swapmovelook", -joy_swapmovelook.value);
		break;
	case CONTROLLER_ENABLE:
		Cvar_SetValue("joy_enable", -joy_enable.value);
		break;
	default:
		break;
	}
...
void M_Controller_Draw(void)
{
	...
	const char* title2 = LOC_GetString("$menu_controller_title");
	...
	for (i = 0; i < CONTROLLER_ITEMS; i++)
	{
		...
		{
		case CONTROLLER_DEADZONE_LOOK:
			text = LOC_GetString("$menu_controller_deadzone_look");
			r = joy_deadzone_look.value; // (joy_deadzone_look.value - 1) / 10; // range
			M_DrawSlider(186, y, r, joy_deadzone_look.value, "%.3f");
			break;

		case CONTROLLER_DEADZONE_MOVE:
			text = LOC_GetString("$menu_controller_deadzone_move");
			r = joy_deadzone_move.value;
			M_DrawSlider(186, y, r, joy_deadzone_move.value, "%.3f");
			break;

		case CONTROLLER_OUTER_THRESHOLD_LOOK:
			text = LOC_GetString("$menu_controller_outer_threshold_look");
			r = joy_outer_threshold_look.value;
			M_DrawSlider(186, y, r, joy_outer_threshold_look.value, "%.3f");
			break;

		case CONTROLLER_OUTER_THRESHOLD_MOVE:
			text = LOC_GetString("$menu_controller_outer_threshold_move");
			r = joy_outer_threshold_move.value;
			M_DrawSlider(186, y, r, joy_outer_threshold_move.value, "%.3f");
			break;

		case CONTROLLER_DEADZONE_TRIGGER:
			text = LOC_GetString("$menu_controller_deadzone_trigger");
			r = joy_deadzone_trigger.value;
			M_DrawSlider(186, y, r, joy_deadzone_trigger.value, "%.3f");
			break;

		case CONTROLLER_SENSITIVITY_YAW:
			text = LOC_GetString("$menu_controller_sensitivity_yaw");
			r = joy_sensitivity_yaw.value / 360.0f;
			M_DrawSlider(186, y, r, joy_sensitivity_yaw.value, "%.3f");
			break;

		case CONTROLLER_SENSITIVITY_PITCH:
			text = LOC_GetString("$menu_controller_sensitivity_pitch");
			r = joy_sensitivity_pitch.value / 360.0f;
			M_DrawSlider(186, y, r, joy_sensitivity_pitch.value, "%.3f");
			break;

		case CONTROLLER_EXPONENT:
			text = LOC_GetString("$menu_controller_exponent");
			r = joy_exponent.value / 10.0f;
			M_DrawSlider(186, y, r, joy_exponent.value, "%.3f");
			break;

		case CONTROLLER_EXPONENT_MOVE:
			text = LOC_GetString("$menu_controller_exponent_move");
			r = joy_exponent_move.value / 10.0f;
			M_DrawSlider(186, y, r, joy_exponent_move.value, "%.3f");
			break;
		case CONTROLLER_INVERT:
			text = LOC_GetString("$menu_controller_invert");
			M_DrawCheckbox(178, y, joy_invert.value < 0);
			break;
		case CONTROLLER_SWAPMOVELOOK:
			text = LOC_GetString("$menu_controller_swapmovelook");
			M_DrawCheckbox(178, y, joy_swapmovelook.value < 0);
			break;
		case CONTROLLER_ENABLE:
			text = LOC_GetString("$menu_controller_enable");
			M_DrawCheckbox(178, y, joy_enable.value < 0);
			break;
		default:
			break;
		}
		...
		if (text)
		{
			if (controllermenu.search.len > 0 &&
				q_strcasestr(text, controllermenu.search.text))
			{
				M_PrintHighlight(16, y, text,
					controllermenu.search.text,
					controllermenu.search.len);
			}
			else
			{
				M_Print(16, y, text);
	...
	M_DrawCharacter(168, 48 + controller_cursor * 8, 12 + ((int)(realtime * 4) & 1));

	if (controllermenu.search.len > 0)
	{
		M_DrawTextBox(16, 170, 32, 1);
		M_PrintHighlight(24, 178, controllermenu.search.text,
			controllermenu.search.text,
			controllermenu.search.len);
		int cursor_x = 24 + 8 * controllermenu.search.len;
		if (numberOfControllerItems == 0)
			M_DrawCharacter(cursor_x, 178, 11 /*^ 128*/);
		else
			M_DrawCharacter(cursor_x, 178, 10 + ((int)(realtime * 4) & 1));
	}
...
void M_Controller_Key(int k)
{
	...
	else if (k == K_BACKSPACE)
	{
		if (controllermenu.search.len > 0)
		{
			controllermenu.search.text[--controllermenu.search.len] = 0;
			if (controllermenu.search.len > 0)
			{
				numberOfControllerItems = 0;
				for (int i = 0; i < CONTROLLER_ITEMS; i++)
				{
					const char* itemtext = M_Controller_GetItemText(i);
					if (itemtext && q_strcasestr(itemtext, controllermenu.search.text))
					{
						numberOfControllerItems++;
						if (numberOfControllerItems == 1)
							controller_cursor = i;
					}
				}
			}
	...
	{
		if (controllermenu.search.len < sizeof(controllermenu.search.text) - 1)
		{
			controllermenu.search.text[controllermenu.search.len++] = k;
			controllermenu.search.text[controllermenu.search.len] = 0;
	...
	case K_MOUSE1:
		...
		if (controllermenu.search.len > 0 && m_mousey >= 170)
			break;

		if (m_mousey >= 48 && m_mousey < 48 + (CONTROLLER_ITEMS * 8))
		{
			controller_cursor = (m_mousey - 48) / 8;

			if (controller_cursor == CONTROLLER_DEADZONE_LOOK ||
				controller_cursor == CONTROLLER_DEADZONE_MOVE ||
				controller_cursor == CONTROLLER_OUTER_THRESHOLD_LOOK ||
				controller_cursor == CONTROLLER_OUTER_THRESHOLD_MOVE ||
				controller_cursor == CONTROLLER_DEADZONE_TRIGGER ||
				controller_cursor == CONTROLLER_SENSITIVITY_YAW ||
				controller_cursor == CONTROLLER_SENSITIVITY_PITCH ||
				controller_cursor == CONTROLLER_EXPONENT ||
				controller_cursor == CONTROLLER_EXPONENT_MOVE)
			{
	...
	case K_UPARROW:
		...
		if (controller_cursor < 0)
			controller_cursor = numberOfControllerItems - 1;
		break;

	case K_DOWNARROW:
		S_LocalSound("misc/menu1.wav");
		controller_cursor++;
		if (controller_cursor >= numberOfControllerItems)
			controller_cursor = 0;
		break;
...
void M_Controller_Mousemove(int cx, int cy)
{
	if (controller_slider_grab)
	{
		...
		{
		case CONTROLLER_DEADZONE_LOOK:
			f = M_ControllerToSliderFraction(cx - 187);
			Cvar_SetValue("joy_deadzone_look", f);
			break;
		...
		case CONTROLLER_SENSITIVITY_YAW:
			f = M_ControllerToSliderFraction(cx - 187) * 360.0f;
			Cvar_SetValue("joy_sensitivity_yaw", f);
			break;
		...
		case CONTROLLER_EXPONENT:
			f = M_ControllerToSliderFraction(cx - 187) * 10;
			Cvar_SetValue("joy_exponent", f);
			break;
	...
	if (controllermenu.search.len > 0 && cy >= 170)
		return;

	int item = (cy - 48) / 8;

	if (item >= 0 && item < CONTROLLER_ITEMS)
	{
		controller_cursor = item;
	}
}
...
static const char* M_Mouse_GetItemText(int index)
{
	...
	{
	case MOUSE_SPEED:
		return LOC_GetString("$menu_mouse_speed");
	case MOUSE_INVERT:
		return LOC_GetString("$menu_invert_mouse");
	case MOUSE_ALWAYSMLOOK:
		return LOC_GetString("$menu_mouse_look");
#ifdef MACOS_X_ACCELERATION_HACK
	case MOUSE_ACCELERATION:
		return LOC_GetString("$menu_acceleration");
#endif
	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
...
static void M_Mouse_AdjustSliders(int dir)
{
	...
	case MOUSE_PITCHMODE:
		if (cl_maxpitch.value >= 89)
			M_Mouse_SetPitchMode(true);
		else
			M_Mouse_SetPitchMode(false);
		break;
#ifdef MACOS_X_ACCELERATION_HACK
	case MOUSE_ACCELERATION:
...
void M_Mouse_Draw(void)
{
	...
	const char* title2 = LOC_GetString("$menu_mouse_title");
	M_PrintWhite((320 - 8 * strlen(title2)) / 2, 32, title2);
	...
	for (i = 0; i < MOUSE_ITEMS; i++)
	{
		...
		{
		case MOUSE_SPEED:
			text = LOC_GetString("$menu_mouse_sensitivity");
			r = (sensitivity.value - 1) / 10;
			M_DrawSlider(186, y, r, sensitivity.value, "%.1f");
			break;

		case MOUSE_INVERT:
			text = LOC_GetString("$menu_mouse_invert");
			M_DrawCheckbox(178, y, m_pitch.value < 0);
			break;

		case MOUSE_ALWAYSMLOOK:
			text = LOC_GetString("$menu_mouse_look");
			M_DrawCheckbox(178, y, in_mlook.state & 1);
			break;

		case MOUSE_PITCHMODE:
			text = LOC_GetString("$menu_mouse_pitchmode");
			if (cl_maxpitch.value >= 89)
				value = LOC_GetString("$menu_mouse_pitch_qs");
			else
				value = LOC_GetString("$menu_mouse_pitch_trad");
			M_Print(178, y, value);
			break;
...
static enum graphics_e
{
	...
	GRAPHICS_ROCKETLIGHT,
	GRAPHICS_EXPLOSIONLIGHT,
	GRAPHICS_MODELLERP,
	GRAPHICS_RENDERSCALE,
	GRAPHICS_CLASSICPARTICLES,
	GRAPHICS_CUSTOMPARTICLES,
	GRAPHICS_COLOREDLIGHTING,
...
static const char* M_Graphics_GetItemText(int index)
{
	...
	case GRAPHICS_BRIGHTNESS:
		return LOC_GetString("$menu_graphics_brightness");
	case GRAPHICS_CONTRAST:
		return LOC_GetString("$menu_graphics_contrast");
	case GRAPHICS_FILTERING:
		return LOC_GetString("$menu_graphics_filtering");
	case GRAPHICS_ANTIALIASING:
		return LOC_GetString("$menu_graphics_antialiasing");
	case GRAPHICS_EXTERNALTEX:
		return LOC_GetString("$menu_graphics_externaltex");
	case GRAPHICS_REPLACEMENTMODELS:
		return LOC_GetString("$menu_graphics_replacementmodels");
	case GRAPHICS_MODELLERP:
		return LOC_GetString("$menu_graphics_modellerp");
	case GRAPHICS_RENDERSCALE:
		return LOC_GetString("$menu_graphics_renderscale");
	case GRAPHICS_CLASSICPARTICLES:
		return LOC_GetString("$menu_graphics_classicparticles");
	case GRAPHICS_ALIASSHADOW:
		return LOC_GetString("$menu_graphics_aliasshadow");
	case GRAPHICS_BRUSHSHADOW:
		return LOC_GetString("$menu_graphics_brushshadow");
	case GRAPHICS_CUSTOMPARTICLES:
		return LOC_GetString("$menu_graphics_customparticles");
	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
	}
...
void M_Graphics_Draw(void)
{
	...
	const char* title2 = LOC_GetString("$menu_graphics_options");
	M_PrintWhite((320 - 8 * strlen(title2)) / 2, 32, title2);
	...
	for (i = 0; i < GRAPHICS_ITEMS; i++)
	{
		...
		{
		case GRAPHICS_BRIGHTNESS:
			text = LOC_GetString("$menu_graphics_brightness");
			r = (1.0 - vid_gamma.value) / 0.5;
			M_DrawSlider(186, y, r, 10.f * r, "%.0f");
			break;

		case GRAPHICS_CONTRAST:
			text = LOC_GetString("$menu_graphics_contrast");
			r = vid_contrast.value - 1.0;
			M_DrawSlider(186, y, r, 10.f * r, "%.0f");
			break;

		case GRAPHICS_FILTERING:
			text = LOC_GetString("$menu_graphics_filtering");
		...
		case GRAPHICS_ANTIALIASING:
			text = LOC_GetString("$menu_graphics_antialiasing");
			if (vid_fsaa.value == 0)
				value = "off";
			else
		...
		case GRAPHICS_EXTERNALTEX:
			text = LOC_GetString("$menu_graphics_externaltex");
			M_DrawCheckbox(178, y, !!gl_load24bit.value);
			break;

		case GRAPHICS_REPLACEMENTMODELS:
			text = LOC_GetString("$menu_graphics_replacementmodels");
			M_DrawCheckbox(178, y, !!*r_replacemodels.string);
			break;

		case GRAPHICS_MODELLERP:
			text = LOC_GetString("$menu_graphics_modellerp");
			M_DrawCheckbox(178, y, !!r_lerpmodels.value && !!r_lerpmove.value);
			break;

		case GRAPHICS_RENDERSCALE:
			text = LOC_GetString("$menu_graphics_renderscale");
			if (r_scale.value == 1)
				M_Print(178, y, LOC_GetString("$menu_native_1_1"));
			else if (r_scale.value == 2)
				M_Print(178, y, LOC_GetString("$menu_half_1_2"));
			else if (r_scale.value == 3)
				M_Print(178, y, LOC_GetString("$menu_third_1_3"));
			else if (r_scale.value == 4)
				M_Print(178, y, LOC_GetString("$menu_quarter_1_4"));
			else
				M_Print(178, y, LOC_GetString("$menu_unknown"));
			break;

		case GRAPHICS_CLASSICPARTICLES:
			text = LOC_GetString("$menu_graphics_classicparticles");
			value = r_particles.value == 1 ? "round (winquake)" : "square (glquake)";
			M_Print(178, y, value);
			break;

		case GRAPHICS_ALIASSHADOW:
			text = LOC_GetString("$menu_graphics_aliasshadow");
			r = r_shadows.value;
			M_DrawSlider(186, y, r, r_shadows.value, "%.1f");
			break;

		case GRAPHICS_BRUSHSHADOW:
			text = LOC_GetString("$menu_graphics_brushshadow");
			M_DrawCheckbox(178, y, r_shadows_bmodels.value != 0);
			break;

		case GRAPHICS_CUSTOMPARTICLES:
			text = LOC_GetString("$menu_graphics_customparticles");
			if (Q_strcmp(r_particledesc.string, "qssm") == 0)
				value = "qssm";
			else if (Q_strcmp(r_particledesc.string, "qssmc") == 0)
				value = "classic+qssm";
...
static const char* M_Sound_GetItemText(int index)
{
	...
	{
	case SOUND_VOLUME:
		return LOC_GetString("$menu_sound_volume");
	case SOUND_MUSICVOL:
		return LOC_GetString("$menu_music_volume");
	case SOUND_MUSICEXT:
		return LOC_GetString("$menu_external_music");
	case SOUND_AUDIORATE:
		return LOC_GetString("$menu_audio_rate");
	case SOUND_WATERFX:
		return LOC_GetString("$menu_water_fx");
	case SOUND_AMBIENTLEVEL:
		return LOC_GetString("$menu_ambient_level");
	case SOUND_STOPSOUND:
		return LOC_GetString("$menu_stop_sound");
	case SOUND_MUTE:
		return LOC_GetString("$menu_mute");
	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
	}
...
static const char* M_Game_GetItemText(int index)
{
	...
	{
	case GAME_ALWAYSRUN:
		return LOC_GetString("$menu_always_run");
	case GAME_ROLLANGLE:
		return LOC_GetString("$menu_strafe_angle_tilt");
	case GAME_FOV:
		return LOC_GetString("$menu_field_of_view");
	case GAME_FLASHES:
		return LOC_GetString("$menu_screen_flashes");
	case GAME_WEAPONBOB:
		return LOC_GetString("$menu_weapon_bob");
	case GAME_DAMAGEKICK:
		return LOC_GetString("$menu_damage_kick");
	case GAME_DAMAGETINT:
		return LOC_GetString("$menu_gun_damage_tint");
	case GAME_AUTOSWITCH:
		return LOC_GetString("$menu_gun_auto_switch");
	case GAME_CONSOLECHAT:
		return LOC_GetString("$menu_console_chat");
	case GAME_SWAPROCKETS:
		return LOC_GetString("$menu_r2g_swap_rockets");
	case GAME_TRUELIGHTNING:
		return LOC_GetString("$menu_true_lightning");
	case GAME_DEADBODYFILTER:
		return LOC_GetString("$menu_deadbody_filter");
	case GAME_MM1MUTE:
		return LOC_GetString("$menu_mute_mm1_chat");
	case GAME_VIEWMODEL:
		return LOC_GetString("$menu_view_model");
	case GAME_TEAMCOLOR:
		return LOC_GetString("$menu_force_team_color");
	case GAME_ENEMYCOLOR:
		return LOC_GetString("$menu_force_enemy_color");
	case GAME_AUTOAIM:
		return  LOC_GetString("menu_gun_auto_aim_indented");
	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
	}
...
static void M_Game_AdjustSliders(int dir)
{
	...
	case GAME_DAMAGEKICK:
		if (v_kickroll.value == 0 || v_kickpitch.value == 0)
		{
...
void M_Game_Draw(void)
{
	...
	const char* title = LOC_GetString("$menu_game_options");
	M_PrintWhite((320 - 8 * strlen(title)) / 2, 4, title);
	...
	for (i = 0; i < GAME_ITEMS; i++)
	{
		...
		case GAME_ALWAYSRUN:
			text = LOC_GetString("$menu_always_run_indented");
			if (cl_alwaysrun.value)
				value = LOC_GetString("$menu_power_bunnyhop");
			else if (cl_forwardspeed.value > 200.0)
				value = LOC_GetString("$menu_traditional");
			else
				value = LOC_GetString("$menu_off_slow");
			M_Print(178, y, value);
			break;

		case GAME_ROLLANGLE:
			text = LOC_GetString("$menu_strafe_angle_tilt_indented");
			M_DrawCheckbox(178, y, cl_rollangle.value != 0);
			break;

		case GAME_FOV:
			text = LOC_GetString("$menu_field_of_view_indented");
			r = (scr_fov.value - 60) / 70.0;  // 70 is range (130-60)
			M_DrawSlider(186, y, r, scr_fov.value, "%.0f");
			break;

		case GAME_FLASHES:
			text = LOC_GetString("$menu_screen_flashes_indented");
			r = gl_cshiftpercent.value / 100.0;
			M_DrawSlider(186, y, r, gl_cshiftpercent.value, "%.0f%%");
			break;

		case GAME_WEAPONBOB:
			text = LOC_GetString("$menu_weapon_bob_indented");
			M_DrawCheckbox(178, y, cl_bob.value != 0);
			break;

		case GAME_DAMAGEKICK:
			text = LOC_GetString("$menu_damage_kick_indented");
			M_DrawCheckbox(178, y, (v_kickroll.value != 0 || v_kickpitch.value != 0));
			break;

		case GAME_DAMAGETINT:
			text = LOC_GetString("$menu_damage_gun_tint_indented");
			if (cl_damagehue.value == 0)
				value = LOC_GetString("$menu_off");
			else if (cl_damagehue.value == 1)
				value = LOC_GetString("$menu_weapon");
			else
				value = LOC_GetString("$menu_weapon_crosshair");
			M_Print(178, y, value);
			break;

		case GAME_AUTOSWITCH:
			text = LOC_GetString("$menu_gun_auto_switch_indented");
			M_DrawCheckbox(178, y, w_switch.value != 0);
			break;

		case GAME_CONSOLECHAT:
			text = LOC_GetString("$menu_console_chat_indented");
			if (cl_say.value == 0)
				value = LOC_GetString("$menu_off");
			else if (cl_say.value == 1)
				value = LOC_GetString("$menu_console");
			else
				value = LOC_GetString("$menu_console_space");
			M_Print(178, y, value);
			break;

		case GAME_SWAPROCKETS:
			text = LOC_GetString("$menu_r2g_swap_rockets_indented");
			M_DrawCheckbox(178, y, cl_r2g.value != 0);
			break;

		case GAME_TRUELIGHTNING:
			text = LOC_GetString("$menu_true_lightning_indented");
			r = cl_truelightning.value / 100.0;
			M_DrawSlider(186, y, r, cl_truelightning.value, "%.0f%%");
			break;

		case GAME_DEADBODYFILTER:
			text = LOC_GetString("$menu_deadbody_filter_indented");
			M_DrawCheckbox(178, y, cl_deadbodyfilter.value != 0);
			break;
		case GAME_MM1MUTE:
			text = LOC_GetString("$menu_mute_mm1_chat_indented");
			M_DrawCheckbox(178, y, con_mm1mute.value != 0);
			break;

		case GAME_TEAMCOLOR:
			text = LOC_GetString("$menu_force_team_color_indented");
		...
		case GAME_ENEMYCOLOR:
			text = LOC_GetString("$menu_force_enemy_color_indented");
	...
	if (game_cursor == GAME_TEAMCOLOR || game_cursor == GAME_ENEMYCOLOR)
		M_PrintRGBA(74, 160, LOC_GetString("$menu_shift_for_rgb_colors"), CL_PLColours_Parse("0xffffff"), 0.6f, false);
...
static const char* M_HUD_GetItemText(int index)
{
	...
	{
	case HUD_CROSSHAIR:
		return LOC_GetString("$menu_crosshair");
	case HUD_SCALE:
	...
	case HUD_MATCHSCORES:
		return LOC_GetString("$menu_show_match_scores");
	case HUD_MATCHCLOCK:
		return LOC_GetString("$menu_match_clock");
	case HUD_SHOWPING:
		return LOC_GetString("$menu_show_ping");
	case HUD_SHOWCLOCK:
		return LOC_GetString("$menu_show_clock");
	case HUD_SHOWSPEED:
		return LOC_GetString("$menu_show_speed");
	case HUD_SHOWSCORES:
		return LOC_GetString("$menu_show_scores");
	case HUD_AUTOID:
		return LOC_GetString("$menu_player_auto_id");
	case HUD_MOVEKEYS:
		return LOC_GetString("$menu_movement_keys");
	case HUD_CONSOLEFONT:
		return LOC_GetString("$menu_console_font_size");
...
static void M_Crosshair_AdjustColor(int dir)
{
	if (keydown[K_SHIFT])
	{
		...
		snprintf(last_crosshair_color, sizeof(last_crosshair_color), "%s", colorStr);
	}
	else
	{
...
static const char* M_Crosshair_GetItemText(int index)
{
	...
	case CROSSHAIR_TOGGLE:
		return LOC_GetString("$menu_use_crosshair");
	case CROSSHAIR_ALPHA:
		return LOC_GetString("$menu_crosshair_alpha");
	case CROSSHAIR_COLOR:
		return LOC_GetString("$menu_crosshair_color");
	case CROSSHAIR_OUTLINE:
		return LOC_GetString("$menu_crosshair_outline");
	case CROSSHAIR_SCALE:
		return LOC_GetString("$menu_crosshair_scale");
	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
	}
...
static const char* M_Console_GetItemText(int index)
{
	...
	case CONSOLE_FONTSIZE:
		return LOC_GetString("$menu_font_size");
	case CONSOLE_HEIGHT:
		return LOC_GetString("$menu_console_height");
	case CONSOLE_SPEED:
		return LOC_GetString("$menu_down_up_speed");
	case CONSOLE_TRANSPARENCY:
		return LOC_GetString("$menu_transparency");
	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
	}
...
static const char* M_Extras_GetItemText(int index)
{
	...
	{
	case EXTRAS_YIELD:
		return LOC_GetString("$menu_system_throttle");
	case EXTRAS_DEMOREEL:
		return LOC_GetString("$menu_start_demo_attract");
	case EXTRAS_NETEXTENSIONS:
		return LOC_GetString("$menu_protocol_exts");
	case EXTRAS_QCEXTENSIONS:
		return LOC_GetString("$menu_qc_extensions");
	case EXTRAS_PREDICTION:
		return LOC_GetString("$menu_prediction");
	case EXTRAS_AUTODEMO:
		return LOC_GetString("$menu_auto_demo");
	case EXTRAS_SPAWNTRAINER:
		return LOC_GetString("$menu_spawn_trainer");
	case EXTRAS_ITEMBOB:
		return LOC_GetString("$menu_q3_item_bobbing");
	case EXTRAS_RESETCONFIG:
		return LOC_GetString("$menu_reset_config");
	case EXTRAS_STARTUP:
		return LOC_GetString("$menu_start_up_screen");
	default:
		q_snprintf(buffer, sizeof(buffer), LOC_GetString("$menu_unknown_item_%d"), index);
		return buffer;
	}
...
static void M_Extras_AdjustSliders(int dir)
{
	...
	case EXTRAS_ITEMBOB:
		...
		break;
	case EXTRAS_RESETCONFIG:
		if (!SCR_ModalMessage(LOC_GetString("$msg_reset_cfg_confirm"), 0.0f))
			break;
...
void M_Extras_Draw(void)
{
	...
	const char* title = LOC_GetString("$menu_miscellaneous_options");
...
static void GetCenteredTextPosition(const char* text, int* out_x, int* out_y)
{
	int max_width = 0;
	int width = 0;
...
json_t* help_json;
jsonentry_t* help_menus;
jsonentry_t* help_item;

int help_items_count;
int help_cursor;
...
void M_Help_Load(void) {
	const char* helpData = (const char*)COM_LoadMallocFile("help.json", NULL);
	if (helpData) {
		help_json = JSON_Parse(helpData);
		free(helpData);
		if (help_json == NULL) {
			return;
		}
		const jsonentry_t* menus = JSON_Find(help_json->root, "Menus", JSON_OBJECT);
		help_menus = help_item = JSON_Find(menus, "Children", JSON_ARRAY);
	}
}
...
void M_Help_Key(int key)
{
	...
	case K_DOWNARROW:
		S_LocalSound("misc/menu1.wav");
		if (help_cursor++ >= help_items_count)
			help_cursor = 0;
		break;
	...
	case K_MOUSE1:
		...
		int index = 0;
		jsonentry_t* entry = help_item->firstchild;
		while (index != help_cursor) {
			index++;
			entry = entry->next;
		}
		jsonentry_t* children = JSON_Find(entry, "Children", JSON_ARRAY);
		if (children == NULL) {
			return;
		}
		help_cursor = 0;
...
void M_Help_Draw(void) {
	...
	jsonentry_t* sub_item = help_item->firstchild;

	while (sub_item) {
		const char* title = JSON_FindString(sub_item, "Title");
		if (title) {
			M_Print(x * invScale, y * invScale, LOC_GetString(title));
			y += 20;
			sub_item = sub_item->next;
		}
		else
		{
			sub_title = JSON_FindString(sub_item, "SubTitle");
			const char* image = JSON_FindString(sub_item, "Image");
			if (image) {
				p = Draw_CachePic(LOC_GetString(image));
				break;
			}
		}
		help_items_count++;
	...
	if (!p) {
		Draw_String(x, 32 - 28, LOC_GetString("$menu_how_to_play"));
		M_Print(x * invScale, y * invScale, LOC_GetString("$menu_back")); y += 20;
	}

	glPopMatrix();

	if (p) {
		int px = (320 / 2) - (p->width / 2);
		M_DrawTransPic(px, 0, p);
		if (sub_title)
		{
			int out_x;
			int out_y;
			const char* text = LOC_GetString(sub_title);
			GetCenteredTextPosition(text, &out_x, &out_y);
			M_PrintWithBreak(out_x, out_y, text, 0, 48);
		}
	}
	else
	{
...
void M_Help_Mousemove(int cx, int cy)
{
	M_UpdateCursor(cy, 32, 20, help_items_count + 1, &help_cursor);
}

void M_Help_GoBack(void) {
	help_cursor = 0;
	if (help_item == help_menus) {
		help_item = help_menus;
		m_state = m_main;
	}
	else
	{
		help_item = help_item->parent->parent->parent;
	}
}
...
json_t* lore_json;
jsonentry_t* lore_menus;
jsonentry_t* lore_item;

int lore_items_count;
int lore_cursor;
...
void M_Lore_Load(void) {
	const char* loreData = (const char*)COM_LoadMallocFile("lore.json", NULL);
	if (loreData) {
		lore_json = JSON_Parse(loreData);
		free(loreData);
		if (lore_json == NULL) {
			return;
		}
		const jsonentry_t* menus = JSON_Find(lore_json->root, "Menus", JSON_OBJECT);
		lore_menus = lore_item = JSON_Find(menus, "Children", JSON_ARRAY);
	}
}
...
void M_Lore_Key(int key)
{
	...
	case K_DOWNARROW:
		S_LocalSound("misc/menu1.wav");
		if (lore_cursor++ >= lore_items_count)
			lore_cursor = 0;
		break;
	...
	case K_MOUSE1:
		...
		int index = 0;
		jsonentry_t* entry = lore_item->firstchild;
		while (index != lore_cursor) {
			index++;
			entry = entry->next;
		}
		jsonentry_t* children = JSON_Find(entry, "Children", JSON_ARRAY);
...
void M_Lore_Draw(void) {
	...
	qpic_t* p = NULL;
	const char* sub_title = NULL;
	jsonentry_t* sub_item = lore_item->firstchild;
	...
	if (!p) {
		Draw_String(x, 32 - 28, LOC_GetString("$menu_lore"));
	}
...
void M_Lore_Mousemove(int cx, int cy)
{
	M_UpdateCursor(cy, 32, 20, lore_items_count + 1, &lore_cursor);
}

void M_Lore_GoBack(void) {
	lore_cursor = 0;
	if (lore_item == lore_menus) {
		lore_item = lore_menus;
		m_state = m_main;
	}
	else
	{
		lore_item = lore_item->parent->parent->parent;
	}
}
...
void M_GameOver_Key(int key)
{
	...
	case K_DOWNARROW:
		S_LocalSound("misc/menu1.wav");
		if (gameover_cursor++ >= 3)
			gameover_cursor = 0;
		break;

	case K_UPARROW:
		S_LocalSound("misc/menu1.wav");
		if (--gameover_cursor < 0)
			gameover_cursor = 2;
		break;
	case K_ENTER:
	case K_KP_ENTER:
...
void M_GameOver_Draw(void) {
	...
	M_Print(x * invScale, y * invScale, LOC_GetString("$menu_quickload")); y += 20;
	M_Print(x * invScale, y * invScale, LOC_GetString("$menu_standardload")); y += 20;
	M_Print(x * invScale, y * invScale, LOC_GetString("$menu_restart")); y += 20;

	Draw_String(x, 32 - 28, LOC_GetString("$menu_gameover"));
...
void M_GameOver_Mousemove(int cx, int cy)
{
	M_UpdateCursor(cy, 32, 20, 3, &gameover_cursor);
}
...
void M_Campaign_Draw(void)
{
	...
	if (campaignmenu.list.search.len > 0)
	{
		...
		int cursor_x = 24 + 8 * campaignmenu.list.search.len;
...
void M_Horde_Draw(void)
{
	...
	if (hordemenu.list.search.len > 0)
	{
		...
		int cursor_x = 24 + 8 * hordemenu.list.search.len;
...
void M_Quit_Draw(void) //johnfitz -- modified for new quit message -- woods modified for match quit warning #matchquit
{
	...
	const char* msg3 = LOC_GetString("$msg_press_y_to_quit");
	int		boxlen;
...
#define NUM_LANCONFIG_CMDS_JOINGAME 7
...
void M_Menu_LanConfig_f(void)
{
	...
	if (StartingGame)
	{
		...
		case 999:
			...
			break;
		default:
			lanConfig_protocol_cursor = 0;
			break;
		}

		if (sv_protocol_pext2)
		{
			lanConfig_protocol_cursor += 3;
		}
	}
	else
	{
...
void M_LanConfig_Draw(void)
{
	...
	startJoin = StartingGame ? LOC_GetString("$menu_new_game") : LOC_GetString("$menu_join_game");
	...
	if (StartingGame) {
		...
		M_Print(basex + 9 * 9 + 1, y, steamServerDescription);
	...
	if (JoiningGame || !lanConfig_steam_server)
	{
		...
		{
			M_Print(basex + (9 * 8) + 10, y, LOC_GetString("$menu_none_known"));
			y += 8;
		}
		else
		{
			...
			if (cl_contentfilter.value && !ip_temporarily_visible[0])
			{
				M_Print(basex + (9 * 8) + 10, y, LOC_GetString("$menu_local_click_to_view"));
				ip_clickables[0].width = 13 * 8;
			}
			else
			{
				M_Print(basex + (9 * 8) + 10, y, va(LOC_GetString("$menu_local_colon_s"), cached_addresses[0]));
			}
			y += 8;
			...
			if (cl_contentfilter.value && !ip_temporarily_visible[1])
			{
				M_Print(basex + (9 * 8) + 10, y, LOC_GetString("$menu_ext_click_to_view"));
				ip_clickables[1].width = 13 * 8;
			}
			else
			{
				M_Print(basex + (9 * 8) + 10, y, va(LOC_GetString("$menu_ext_colon_s"), my_public_ip));
			}
		}

		y += 16;
		M_Print(basex, y, LOC_GetString("$menu_port_colon"));
		M_DrawTextBox(basex + 8 * 10, y - 8, 6, 1);
		M_Print(basex + 9 * 10, y, lanConfig_portname);
		if (StartingGame && lanConfig_cursor == 1 || JoiningGame && lanConfig_cursor == 0)
		{
		...
		{
			y += 8;
			M_Print(basex, y, LOC_GetString("$menu_protocol_colon"));
		...
		if (JoiningGame)
		{
			...
			if (lanConfig_cursor == 4)
				M_DrawCharacter(basex - 8, y, 12 + ((int)(realtime * 4) & 1));
			y += 8;
			M_Print(basex, y, LOC_GetString("$menu_bookmarks"));
			...
			y += 8;
			M_Print(basex, y, LOC_GetString("$menu_if_you_cant_sorry"));
			y += 8;
			M_Print(basex, y, LOC_GetString("$menu_use_hamachi_or_radmin"));
			y += 8;
			M_Print(basex, y, " ");
			y += 4;
#endif
		...
		if (realtime < copy_message_time)
		{
			...
			snprintf(copy_message, sizeof(copy_message), LOC_GetString("$menu_copied_colon_s"), last_copied_ip);
...
void M_LanConfig_Key(int key)
{
	...
	if (StartingGame && (!lanConfig_steam_server && lanConfig_cursor >= 4 || lanConfig_steam_server && lanConfig_cursor >= 2))
	{
		//if (key == K_UPARROW)
		//	lanConfig_cursor = 1;
		//else
...
void M_History_Draw(void)
{
	...
	Draw_String(x, y - 28, LOC_GetString("$menu_history"));
	M_DrawQuakeBar(x - 8, y - 16, cols + 2);
	...
	for (i = 0; i < numvis; i++)
	{
		...
		if (cls.state == ca_connected && lanConfig_port == DEFAULTnet_hostport)
		{
	...
	M_PrintWhite(x, y + 2 + historymenu.list.viewsize * 8 + 10, LOC_GetString("$menu_ctrl_backspace_delete"));
}
...
void M_Bookmarks_Draw(void)
{
	...
	Draw_String(x, y - 28, LOC_GetString("$menu_bookmarks"));
	M_DrawQuakeBar(x - 8, y - 16, cols + 2);
	...
	M_Print(x, y + 2 + bookmarksmenu.list.viewsize * 8 + 20, LOC_GetString("$menu_keys_add_edit_del"));
}
...
{
	M_Print(10, 40, LOC_GetString("$menu_hostname_ip"));
	M_DrawTextBox(6, 46, 38, 1);
	M_PrintWhite(14, 54, temp_name);

	M_Print(10, 72, LOC_GetString("$menu_bookmark_name"));
	M_DrawTextBox(6, 78, 38, 1);
	M_PrintWhite(14, 86, temp_alias);

	M_DrawTextBox(6, 106 - 8, 14, 1);
	M_Print(15, 106, LOC_GetString("$menu_accept_changes"));
...
void M_GameOptions_Draw(void)
{
	...
	M_Print(160, y, LOC_GetString("$menu_begin_game"));
	y += 16;

	M_Print(0, y, LOC_GetString("$menu_max_players"));
	M_Print(160, y, va("%i", maxplayers));
	y += 8;

	M_Print(0, y, LOC_GetString("$menu_public"));
	M_Print(160, y, sv_public.value ? LOC_GetString("$menu_yes") : LOC_GetString("$menu_no"));
	y += 8;

	M_Print(0, y, LOC_GetString("$menu_game_type"));
	M_Print(160, y, coop.value ? LOC_GetString("$menu_cooperative") : LOC_GetString("$menu_deathmatch"));
	y += 8;

	M_Print(0, y, LOC_GetString("$menu_teamplay"));
	if (rogue)
	{
		const char* msg;
		...
		case 3: msg = LOC_GetString("$menu_tag"); break;
		case 4: msg = LOC_GetString("$menu_capture_the_flag"); break;
		case 5: msg = LOC_GetString("$menu_one_flag_ctf"); break;
		case 6: msg = LOC_GetString("$menu_three_team_ctf"); break;
	...
	M_Print(0, y, LOC_GetString("$menu_skill"));
	if (!coop.value && !HasBots())
	{
		M_PrintRGBA(160, y, LOC_GetString("$menu_normal_difficulty"), CL_PLColours_Parse("0xffffff"), 0.5f, true);
	}
	else if (skill.value == 0)
		M_Print(160, y, LOC_GetString("$menu_easy_difficulty"));
	else if (skill.value == 1)
		M_Print(160, y, LOC_GetString("$menu_normal_difficulty"));
	else if (skill.value == 2)
		M_Print(160, y, LOC_GetString("$menu_hard_difficulty"));
	else
		M_Print(160, y, " LOC_GetString($menu_nightmare_difficulty");
	y += 8;

	M_Print(0, y, LOC_GetString("$menu_frag_limit"));
	M_Print(160, y, fraglimit.value == 0 ? LOC_GetString("$menu_none") : va(LOC_GetString("$menu_i_frags"), (int)fraglimit.value));
	y += 8;

	M_Print(0, y, LOC_GetString("$menu_time_limit"));
	M_Print(160, y, timelimit.value == 0 ? LOC_GetString("$menu_none") : va(LOC_GetString("$menu_i_minutes"), (int)timelimit.value));
	y += 8;

	y += 8;

	M_Print(0, y, LOC_GetString("$menu_episode"));
	if (hipnotic)
	...
	if (hipnotic)
	{
		...
		if (m_skill_mapname[0])
			M_PrintRGBA(160, y + 8, hipnoticlevels[hipnoticepisodes[startepisode].firstLevel + startlevel].name, CL_PLColours_Parse("0xffffff"), 0.5, false);
		else
	...
	else if (rogue)
	{
		...
		if (m_skill_mapname[0])
			M_PrintRGBA(160, y + 8, roguelevels[rogueepisodes[startepisode].firstLevel + startlevel].name, CL_PLColours_Parse("0xffffff"), 0.5, false);
		else
	...
	M_Print(0, y, LOC_GetString("$menu_custom_level"));
	if (m_skill_mapname[0])
		M_PrintWhite(160, y, m_skill_mapname);
	else
		M_Print(160, y, LOC_GetString("$menu_ellipsis"));
	y += 8;
...
void M_GameOptions_Key(int key)
{
	...
	case K_MOUSE1: // woods #mousemenu
		...
		if (gameoptions_cursor == 0)
		{
			...
			Cbuf_AddText(va("steamserver %d\n", lanConfig_steam_server));
...
void M_Search_Draw(void)
{
	...
	M_Print(x, 40, LOC_GetString("$menu_searching_ellipsis"));
	...
	M_PrintWhite((320 / 2) - ((22 * 8) / 2), 64, LOC_GetString("$menu_no_quake_servers_found"));
...
{
	char bufTimestamp[20], bufLastQuery[20];
	Q_strncpy(bufTimestamp, timestamp, 19);
	bufTimestamp[19] = '\0';
	Q_strncpy(bufLastQuery, lastQuery, 19);
	bufLastQuery[19] = '\0';

	if (Q_strcmp(bufTimestamp, bufLastQuery) == 0)
		*status = true;
	else
		*status = false;
}
...
void FetchAndSortServers(void)
{
	...
	else
	{
		...
		CurlServerList(&serversmenu.items, &actualServerCount);
	}
...
void M_ServerList_Draw(void)
{
	...
	Draw_String(x, y - 28, LOC_GetString("$menu_servers"));
	M_DrawQuakeBar(x - 8, y - 16, cols + 2);
...
void M_Mods_Draw(void)
{
	...
	Draw_String(x, y - 28, LOC_GetString("$menu_mods"));
	M_DrawQuakeBar(x - 8, y - 16, cols + 2);
...
//static inline void Truncate(char* s, int max)
//{
//	size_t len = strlen(s);
//	if (len > max) {
//		s[max - 4] = '.';
...
#define WORKSHOP_MAX_VIS_MODS	19
...
static struct
{
	...
	menuticker_t		ticker;
	qboolean			scrollbar_grab;
	workshop_moditem_t* items;
} workshop_modsmenu;

static void M_Workshop_Mods_Add(
	const char* id,
	const char* title,
	const char* owner,
	const char* description,
...
{
	workshop_moditem_t mod;
	mod.id = atoll(id);
	strncpy(mod.title, title, sizeof(mod.title));
	strncpy(mod.owner, owner, sizeof(mod.owner));
	strncpy(mod.description, description, sizeof(mod.description));
	strncpy(mod.creation, creation, sizeof(mod.creation));
	mod.subscribed = atoi(subscribed);
	char directory[MAX_PATH]; q_snprintf(directory, sizeof(directory), "workshop_%s", id); Sys_mkdir(directory);
	char filename[MAX_PATH]; q_snprintf(filename, sizeof(filename), "%s/preview.png", directory);
	if (Sys_FileType(filename) == FS_ENT_NONE) {
		Curl_DownloadFile(preview_url, "preview.png", filename);
	}
	char relative_filename[MAX_PATH]; q_snprintf(relative_filename, sizeof(relative_filename), "../%s", filename);
	strncpy(mod.preview, relative_filename, sizeof(mod.preview));
	VEC_PUSH(workshop_modsmenu.items, mod);
	workshop_modsmenu.items[workshop_modsmenu.list.numitems] = mod;
	workshop_modsmenu.list.numitems++;
}

char m_selected_mod;

#define WORKSHOP_FIELD_SIZE 65535

static void M_Workshop_Mods_Init(void)
{
	m_selected_mod = -1;

	workshop_modsmenu.list.viewsize = MAX_VIS_MODS;
	workshop_modsmenu.list.cursor = -1;
	workshop_modsmenu.list.scroll = 0;
	workshop_modsmenu.list.numitems = 0;
	workshop_modsmenu.modcount = 0;
	workshop_modsmenu.scrollbar_grab = false;
	VEC_CLEAR(workshop_modsmenu.items);

	M_Ticker_Init(&workshop_modsmenu.ticker);

	Pipe_Write("workshop_mods");
	for (;;) {
		if (!Pipe_Read()) break;
		if (pipe_buffer[0] == '\x04') break;

		char id[WORKSHOP_FIELD_SIZE];
		strncpy(id, pipe_buffer, sizeof(id) - 1);
		id[sizeof(id) - 1] = '\0';

		if (!Pipe_Read()) break;
		char title[WORKSHOP_FIELD_SIZE];
		strncpy(title, pipe_buffer, sizeof(title) - 1);
		title[sizeof(title) - 1] = '\0';

		if (!Pipe_Read()) break;
		char owner[WORKSHOP_FIELD_SIZE];
		strncpy(owner, pipe_buffer, sizeof(owner) - 1);
		owner[sizeof(owner) - 1] = '\0';

		if (!Pipe_Read()) break;
		char description[WORKSHOP_FIELD_SIZE];
		strncpy(description, pipe_buffer, sizeof(description) - 1);
		description[sizeof(description) - 1] = '\0';

		if (!Pipe_Read()) break;
		char preview_url[WORKSHOP_FIELD_SIZE];
		strncpy(preview_url, pipe_buffer, sizeof(preview_url) - 1);
		preview_url[sizeof(preview_url) - 1] = '\0';

		if (!Pipe_Read()) break;
		char creation[WORKSHOP_FIELD_SIZE];
		strncpy(creation, pipe_buffer, sizeof(creation) - 1);
		creation[sizeof(creation) - 1] = '\0';

		if (!Pipe_Read()) break;
		char subscribed[2];
		strncpy(subscribed, pipe_buffer, 1);
		subscribed[1] = '\0';
	...
	if (workshop_modsmenu.list.cursor == -1) {
		workshop_modsmenu.list.cursor = 0;
	}

	M_List_CenterCursor(&workshop_modsmenu.list);
}

void M_Menu_Workshop_Mods_f(void)
{
	key_dest = key_menu;
	workshop_modsmenu.prev = m_state;
	m_state = m_workshop_mods;
	m_entersound = true;
	M_Workshop_Mods_Init();
}
	...
	float r, float g, float b, float a)
{
	...
	glVertex2f(x2, y);   // bottom-right
	glVertex2f(x2, y2);  // top-right
	glVertex2f(x, y2);  // top-left
	glEnd();

	glPopAttrib();
}

#define DRAW_DOUBLE_RECT(x, y, w, h) \
    DrawRect((x) - 1, (y) - 1, (w) + 2, (h) + 2, 0.5f, 0.5f, 0.5f, 0.5f); \
    DrawRect((x), (y), (w), (h), 0.0f, 0.0f, 0.0f, 0.8f); \
	glPushMatrix(); \

#define END_DOUBLE_RECT \
	glPopMatrix(); \
...
void M_Workshop_Mods_Draw(void)
{
	...
	if (m_selected_mod >= 0) {
		...
		workshop_moditem_t* selected_mod = &workshop_modsmenu.items[m_selected_mod];

		{
			DRAW_DOUBLE_RECT(x, y, 288, 16);
			M_PrintWithLimit(x + 4, y + 4, selected_mod->title, 32); y += 18;
			END_DOUBLE_RECT
		}

		{
			...
			glScalef(scale, scale, scale);
			char owner[1024];
			q_snprintf(owner, sizeof(owner), LOC_GetString("$mod_authoranddate"), selected_mod->owner, selected_mod->creation);
			M_PrintWithLimit((x + 4) * inv_scale, (y + 4) * inv_scale, owner, 32); y += 18;
			END_DOUBLE_RECT
		}

		{
			DRAW_DOUBLE_RECT(x, y, 288, (93 + 2));
			qpic_t* preview = Draw_CachePic(selected_mod->preview);
			if (preview) {
				glPushMatrix();
				float scale = 93.0f / (float)preview->height;
				float inv_scale = 1.0f / scale;
				glScalef(scale, scale, scale);
				Draw_Pic(x * inv_scale, y * inv_scale, preview);
				glPopMatrix();
			}
			y += 93 + 2;
			END_DOUBLE_RECT
		...
		{
			DRAW_DOUBLE_RECT(x, y, 288, 48);
			const float scale = 0.75f;
			const float inv_scale = 1.0f / scale;
			glScalef(scale, scale, scale);
			M_PrintWithBreak((x + 4) * inv_scale, (y + 4) * inv_scale, selected_mod->description, 160, 47); y += 50;
			END_DOUBLE_RECT
		}

		{
			DRAW_DOUBLE_RECT(x, y, 288, 16);
			M_DrawTextBox(x - 4, y - 4, 10, 1);
			M_PrintWithLimit(x + 12 - 2, y + 6 - 3, LOC_GetString("$workshop_activate"), 32); y += 18;
			END_DOUBLE_RECT
		}

		int  f;
		f = (int)(realtime * 10) % 6;
		M_DrawTransPic(x, y + 6 - 3, Draw_CachePic(va("gfx/menudot%i.lmp", f + 1)));
	}
	else {

		workshop_modsmenu.x = x;
		workshop_modsmenu.y = y;
		workshop_modsmenu.cols = cols;

		if (!keydown[K_MOUSE1])
			workshop_modsmenu.scrollbar_grab = false;

		if (workshop_modsmenu.prev_cursor != workshop_modsmenu.list.cursor)
		{
			workshop_modsmenu.prev_cursor = workshop_modsmenu.list.cursor;
			M_Ticker_Init(&workshop_modsmenu.ticker);
		}
		else
			M_Ticker_Update(&workshop_modsmenu.ticker);

		Draw_String(x, y - 28, LOC_GetString("$menu_workshop"));
		M_DrawQuakeBar(x - 8, y - 16, cols + 2);

		M_List_GetVisibleRange(&workshop_modsmenu.list, &firstvis, &numvis);
		for (i = 0; i < numvis; i++)
		{
			int idx = i + firstvis;
			qboolean selected = (idx == workshop_modsmenu.list.cursor);

			char text[1024];
			snprintf(text, sizeof(text), "%s - %s", workshop_modsmenu.items[idx].title, workshop_modsmenu.items[idx].creation);

			M_PrintScroll(x, y + i * 8, (cols - 2) * 8, text, selected ? modsmenu.ticker.scroll_time : 0.0, !workshop_modsmenu.items[idx].subscribed);

			if (selected)
				M_DrawCharacter(x - 8, y + i * 8, 12 + ((int)(realtime * 4) & 1));
		}

		if (M_List_GetOverflow(&workshop_modsmenu.list) > 0)
		{
			M_List_DrawScrollbar(&workshop_modsmenu.list, x + cols * 8 - 8, y);

			if (workshop_modsmenu.list.scroll > 0)
				M_DrawEllipsisBar(x, y - 8, cols);
			if (workshop_modsmenu.list.scroll + workshop_modsmenu.list.viewsize < workshop_modsmenu.list.numitems)
				M_DrawEllipsisBar(x, y + workshop_modsmenu.list.viewsize * 8, cols);
		}
	}
}

qboolean M_Workshop_Mods_Match(int index, char initial)
{
	return q_tolower(workshop_modsmenu.items[index].title[0]) == initial;
}

void M_Workshop_Mods_Key(int key)
{
	int x, y; // woods #mousemenu

	if (workshop_modsmenu.scrollbar_grab)
	{
		switch (key)
		{
		case K_ESCAPE:
	...
	if (M_List_Key(&workshop_modsmenu.list, key))
		return;

	if (M_List_CycleMatch(&workshop_modsmenu.list, key, M_Workshop_Mods_Match))
		return;

	if (M_Ticker_Key(&workshop_modsmenu.ticker, key))
		return;
	...
	enter:
		if (m_selected_mod < 0) {
			if (workshop_modsmenu.list.numitems > 0) {
				m_selected_mod = workshop_modsmenu.list.cursor;
			}
		}
		else {
			if (SCR_ModalMessage(LOC_GetString("$msg_activate_mod"), 0.0f)) {
				workshop_moditem_t* selected_mod = &workshop_modsmenu.items[m_selected_mod];
				Pipe_Write("workshop_install %llu", selected_mod->id);
				SCR_ModalMessage(LOC_GetString("$msg_installing_mod"), 1.0);
				if (Pipe_Read()) {
					if (pipe_buffer[0] == '\x04') {
						SCR_ModalMessage(LOC_GetString("$msg_error_mod"), 5.0f);
					}
					else {
						Cbuf_AddText(va("game %s\n", pipe_buffer));
						if (Pipe_Read()) {
	...
	case K_MOUSE1: // woods #mousemenu
		if (m_selected_mod < 0) {
			x = m_mousex - workshop_modsmenu.x - (workshop_modsmenu.cols - 1) * 8;
			y = m_mousey - workshop_modsmenu.y;
			if (x < -8 || !M_List_UseScrollbar(&workshop_modsmenu.list, y))
				goto enter;
			workshop_modsmenu.scrollbar_grab = true;
			M_Workshop_Mods_Mousemove(m_mousex, m_mousey);
		}
		else {
			if (m_mousey >= 190 && m_mousey <= 190 + 12) {
				goto enter;
			}
		}
	default:
...
void M_Workshop_Mods_Mousemove(int cx, int cy) // woods #mousemenu
{
	cy -= workshop_modsmenu.y;
	...
	if (workshop_modsmenu.scrollbar_grab)
	{
		...
		M_List_UseScrollbar(&workshop_modsmenu.list, cy);
	}

	M_List_Mousemove(&workshop_modsmenu.list, cy);
}
...
void M_Demos_Draw(void)
{
	...
	Draw_String(x, y - 28, LOC_GetString("$menu_demos"));
	M_DrawQuakeBar(x - 8, y - 16, cols + 2);
...
} menucommands[] =
{
	...
	{"menu_horde", M_Menu_Horde_f}, // woods
	{"menu_campaign", M_Menu_Campaign_f}, // woods
...
void M_Draw(void)
{
	...
	case m_workshop_mods:
		M_Workshop_Mods_Draw();
		break;
...
void M_Keydown(int key)
{
	...
	case m_autoaim: // woods #skillmenu (iw)
		M_AutoAim_Key(key);
		return;
	...
	case m_workshop_mods: // woods #modsmenu (iw)
		M_Workshop_Mods_Key(key);
		return;
...
void M_Mousemove(int x, int y) // woods #mousemenu
{
	...
	case m_allmods:
		M_AllMods_Mousemove(x, y);
		return;

	case m_language:
		M_Language_Mousemove(x, y);
		return;
	}
}
...
void M_CheckMods(void) // woods #modsmenu (iw)
{
	...
	m_main_mods = 1;/*M_CheckCustomGfx("gfx/menumods.lmp",
		"gfx/mainmenu.lmp", 26888, main_hashes, countof(main_hashes));*/

	m_main_demos = 1; /*M_CheckCustomGfx("gfx/menudemos.lmp", // woods #demosmenu
		"gfx/mainmenu.lmp", 26888, main_hashes, countof(main_hashes));*/

	m_singleplayer_showlevels = 1;/*M_CheckCustomGfx("gfx/sp_maps.lmp",
		"gfx/sp_menu.lmp", 14856, sp_hashes, countof(sp_hashes));*/

	m_skill_usegfx = 1;/*M_CheckCustomGfx("gfx/skillmenu.lmp",
		"gfx/sp_menu.lmp", 14856, sp_hashes, countof(sp_hashes));*/
