...
void Modlist_Init(void)
{
	...
	do
	{
		...
		// Skip workshop_* folders
		if (strncmp(fdat.cFileName, "workshop_", 9) == 0)
			continue;
...
static void Host_Map_f(void)
{
	...
	{
		horde_map = 1;
		if (_strnicmp(game, "workshop_", 9) == 0) {
			char src[MAX_PATH];
			char dst[MAX_PATH];
			q_snprintf(src, sizeof(src), "%s\\%s\\maps\\%s.bsp", com_basedir, game, name);
			q_snprintf(dst, sizeof(dst), "%s\\mg1\\maps\\%s.bsp", com_basedir, name);
			if (!CopyFileA(src, dst, FALSE))
			{
				Con_Printf("Copy failed\n");
			}
		}
		if (_stricmp(game, "mg1") != 0)
		{
			Cbuf_AddText("game mg1\n");
...
char* GetLastSavedFile(char* output, size_t output_size) {
	...
	for (i = 0; i < 20; i++) {
		char filename[16];
		snprintf(filename, sizeof(filename), "s%d.sav", i);

		snprintf(name, sizeof(name), "%s/%s", save_dir, filename);
...
#endif
		{
			if (st.st_mtime > latest_time) {
				...
				strncpy(latest_filename, filename, sizeof(latest_filename));
	...
	if (latest_filename[0] != '\0') {
		strncpy(output, latest_filename, output_size);
...
{
	if (!sv_autoload.value || svs.maxclients != 1 || cl.intermission)
		return false;
	...
	again:
		...
		switch (result) {
		case 0:
			strncpy(load, "quick.sav", sizeof("quick.sav"));
			break;
		case 1:
			M_ToggleMenu(69);
			return true;
...
static void Host_SavegameComment(char* text)
{
	...
	if (i > 22) i = 22;
	memcpy(text, cl.mapname, (size_t)i);
...
static void Host_Savegame_f(void)
{
	...
	q_snprintf(name, sizeof(name), "%s/%s", save_dir, relname);
...
void GetUserConfigDir(char* config_dir)
{
	...
	char config_dir_env[MAX_PATH] = "\0";
...
void GetSaveDir(char* save_dir, char* saved_games_dir)
{
	...
	char save_dir_env[MAX_PATH] = "\0";
	size_t save_dir_len = ExpandEnvironmentStrings(saved_games_dir_internal, save_dir_env, MAX_PATH);
	if (saved_games_dir) {
		strnset(saved_games_dir, '\0', MAX_PATH);
		strncpy(saved_games_dir, save_dir_env, MAX_PATH);
	}
...
static void Host_Loadgame_f(void)
{
	...
	q_snprintf(name, sizeof(name), "%s/%s", save_dir, relname); // woods #autoload (iw)
