...
================
*/
void Draw_Character(int x, int y, Uint32 num)
{
	if (num == 32)
	{
...
================
*/
void Draw_CharacterRGBA(int x, int y, Uint32 num, plcolour_t c, float alpha)
{
	int				row, col;
	float			frow, fcol, size;
...
void Draw_Character_Rotation(int x, int y, Uint32 num, int rotation) // woods #movementkeys
{
	//num &= 255;
...
/*
================
Draw_String_Right -- avi�o - draws string right aligned
================
*/
...
void Draw_StringRGBA(int x, int y, const char* str, plcolour_t c, float alpha)
{
	...
	if (c.type == 2) {
		...
		unsigned char* pal = (unsigned char*)(&d_8to24table[(c.basic << 4) + 8]);
		color[0] = pal[0];
		color[1] = pal[1];
		color[2] = pal[2];
		color[3] = alpha * 255;
...
void GL_SetCanvas(canvastype newcanvas)
{
	...
	case CANVAS_TOPRIGHT4: // woods #hud_diff
		...
		break;
	case CANVAS_SCOREBOARD_SA: // avi�o
