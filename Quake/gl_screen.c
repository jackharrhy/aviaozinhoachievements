...
void SCR_DrawCenterStringBottom(void)
{
	...
	realobs = Info_GetKey(
		cl.scores[cl.realviewentity - 1].userinfo,
		"observer",
		buf,
		sizeof(buf));

	star_realobs = Info_GetKey(
		cl.scores[cl.realviewentity - 1].userinfo,
		"*observer",
		buf2,
		sizeof(buf2));
...
#ifdef BDDPRE4
	GL_SetCanvas(CANVAS_SA_SMALL);
#else
	if (!strcmp(cl.observer, "y") && (cl.modtype >= 2))
		GL_SetCanvas(CANVAS_OBSERVER);
	else
		GL_SetCanvas(CANVAS_MOD);
#endif

	if (cl.intermission)
	{
		remaining =
			scr_printspeed.value *
	...
	{
		float fade = q_max(
			con_notifyfade.value *
			con_notifyfadetime.value,
			0.f);

		remaining = 9999;

		alpha = fade ?
			q_min(scr_centertime_off / fade, 1.f) :
			1.f;
	}
...
// aviao: added true vertical centering
...
void SCR_DrawAutoLoad(void) // woods add ^m support
{
	GL_SetCanvas(CANVAS_MENU); //johnfitz
...
int SCR_AutoloadMessage()
{
	...
	do
	{
		...
		case K_DOWNARROW:
			...
			if (m_gameover_cursor++ >= 2)
				m_gameover_cursor = 0;
			break;
		case K_UPARROW:
			S_LocalSound("misc/menu1.wav");
			if (--m_gameover_cursor < 0)
				m_gameover_cursor = 2;
			break;
		case K_ENTER:
		case K_KP_ENTER:
		case K_ABUTTON:
			result = m_gameover_cursor;
			break;
		}
		scr_drawautoload = true;
		SCR_UpdateScreen();
		scr_drawautoload = false;
		Sys_Sleep(16);
	} while (result == -1);
	return result;
}
